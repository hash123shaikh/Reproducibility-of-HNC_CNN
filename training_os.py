...
CONFIGURATIONS = {
    ...
    LOGS_PATH: "results/logs/log_os.txt",  # Directory to store log files
...
random.seed(5708730) # Set a fixed seed for reproducibility
# 2) Random split seed
random_seed_split = random.randint(0, 5381955) # Random seed for splitting data into folds
# 3) Random seed torch
torch.manual_seed(1737769) # Set seed for reproducibility in PyTorch
            ...
            with open(file_path, "w") as o:
                ...
                with contextlib.redirect_stdout(o):
                    ...
                    history = fit(
                        ...
                        dataloaders,                      # Dictionary of all dataloaders (train/val/test)
                        parameters=CONFIGURATIONS[HYPERPARAMETERS],  # Hyperparameters such as learning rate
                        store_model={**CONFIGURATIONS[STORE_MODEL], MODEL_ID: f"os_5-fold_cv_fold{fold}"},  # Configuration for saving the trained model
                    )