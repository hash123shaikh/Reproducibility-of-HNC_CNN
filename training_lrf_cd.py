...
CONFIGURATIONS = {
    ...
    EVENT: LRF,  # Type of event to predict: Distant Metastasis (DM), etc.
    HYPERPARAMETERS: {
        LEARNING_RATE: 0.05,  # Override default learning rate
    },
    BATCH_SIZE: 64,  # Number of samples per training batch
    LOGS_PATH: "results/logs/log_lrf_cd.txt",  # Directory to store log files
...
random.seed(1892101) # Set a fixed seed for reproducibility
# 2) Random split seed
random_seed_split = random.randint(0, 8449678) # Random seed for splitting data into folds
# 3) Random seed torch
torch.manual_seed(4951952) # Set seed for reproducibility in PyTorch
            ...
            with open(file_path, "w") as o:
                ...
                with contextlib.redirect_stdout(o):
                    ...
                    history = fit(
                        ...
                        dataloaders,                      # Dictionary of all dataloaders (train/val/test)
                        parameters=CONFIGURATIONS[HYPERPARAMETERS],  # Hyperparameters such as learning rate
                        store_model={**CONFIGURATIONS[STORE_MODEL], MODEL_ID: f"lrf_cd_5-fold_cv_fold{fold}"},  # Configuration for saving the trained model
                    )
