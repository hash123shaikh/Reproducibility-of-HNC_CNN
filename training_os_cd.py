...
CONFIGURATIONS = {
    ...
    TIME_TO_EVENT: 4 * 365,  # Time period (in days) to consider if the event occurred (e.g., 2 years)
    EVENT: OS,  # Type of event to predict: Distant Metastasis (DM), etc.
    HYPERPARAMETERS: {
        LEARNING_RATE: 0.05,  # Override default learning rate
    },
    BATCH_SIZE: 64,  # Number of samples per training batch
    LOGS_PATH: "results/logs/log_os_cd.txt",  # Directory to store log files
    ...
    STORE_MODEL: {
         ...
         THRESHOLD: 0.7, # Minimum AUC for the validation set
         MAX_DIFFERENCE: 0.05, # Maximum difference between training and validation AUC
    }
}
...
random.seed(1739324) # Set a fixed seed for reproducibility
# 2) Random split seed
random_seed_split = random.randint(0, 2430605) # Random seed for splitting data into folds
# 3) Random seed torch
torch.manual_seed(5430016) # Set seed for reproducibility in PyTorch
            ...
            with open(file_path, "w") as o:
                ...
                with contextlib.redirect_stdout(o):
                    ...
                    history = fit(
                        ...
                        dataloaders,                      # Dictionary of all dataloaders (train/val/test)
                        parameters=CONFIGURATIONS[HYPERPARAMETERS],  # Hyperparameters such as learning rate
                        store_model={**CONFIGURATIONS[STORE_MODEL], MODEL_ID: f"os_cd_5-fold_cv_fold{fold}"},  # Configuration for saving the trained model
                    )
